...
export function Providers({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    sync();
    window.addEventListener("onchain-poaps-theme-change", sync);
    return () => window.removeEventListener("onchain-poaps-theme-change", sync);
  }, []);
  ...
  return (
    <WagmiProvider config={wagmiConfig}>
      <QueryClientProvider client={queryClient}>
        <RainbowKitProvider
          theme={dark ? darkTheme({ accentColor: "#ff5a1f" }) : lightTheme({ accentColor: "#ff5a1f", accentColorForeground: "#0b0d10" })}
          modalSize="compact"
        >
          <ErrorBoundary>
